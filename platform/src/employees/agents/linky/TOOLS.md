...
Use `get_scheduled_posts` to view the content calendar. Returns a compact summary (title, date, status, content preview). Useful for:
- Checking for scheduling conflicts before saving a post
- Answering "what do I have coming up?"
- Reviewing posts for a specific month

Optional filters: `status`, `upcoming_only` ("true"/"false"), `month` (YYYY-MM), `limit` (1-50).
...
*(Add specific notes, preferred voices, or other setup-specific info here as you learn them.)*
